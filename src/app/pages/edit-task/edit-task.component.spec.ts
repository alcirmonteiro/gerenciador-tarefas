...
describe('EditTaskComponent', () => {
  ...
  it('deve editar uma tarefa', () => {
    ...
    const isCompletedCheckBoxChecked = testHelper.isCheckBoxChecked('edit-task-completed');
    expect(isCompletedCheckBoxChecked).toBe(fakeTask.completed);

    const fakeEditedTask: ITaskCreate = {
      title: 'Tarefa Editada',
      completed: true,
    }

    testHelper.triggerInputEvent('edit-task-title', fakeEditedTask.title);
    testHelper.changeCheckBoxValue('edit-task-completed', fakeEditedTask.completed);
